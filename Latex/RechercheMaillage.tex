...
\begin{wrapfigure}{r}{0.5\textwidth}
	\begin{center}
		\includegraphics[width=0.7\linewidth]{../Figures/ex_mesh}
	\end{center}
...
For each $ t $ three pairs are added to the multimap, where the edges $(v_{i_k},v_{i_l}), \, k,l \in \{1,2,3\}, \ k \neq l $, stored by the data type {\ttfamily pair<int,int> } (more precisely {\ttfamily  pair<i\textsubscript{k},i\textsubscript{l}> }), represent the keys, whereas the mapped value is an integer representing the position of the triangle in the array of triangles. Hence, the initialization if the multimap is realized in $ 3n_T $ loops, so in $ O(n_T) $. \\
...
Optionally, the method {\ttfamily Mesh::promenade} takes also a vector with entries of the class {\ttfamily Triangle}, called path, as input variable. The current triangle is pushed back on the vector and, due to recursively calling of the method, the vector stores the consecutively passed triangles when the algorithm terminates. The path is used to visualize the results of the method {\ttfamily Mesh::promenade} in \ref{visualization}.
